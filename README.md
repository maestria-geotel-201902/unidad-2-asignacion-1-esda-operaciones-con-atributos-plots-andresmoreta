...
## Reading layer `PROVCenso2010' from data source `/home/ingan/unidad-2-asignacion-1-esda-operaciones-con-atributos-plots-andresmoreta/data/divisionRD.gpkg' using driver `GPKG'
...
``` r
proven17 %>%
  dplyr::select(contains('A qué cree usted que se debe la delincuencia en su barrio o comunidad: ¿Otra causa?')) %>%
  plot(breaks = 'jenks')
```
...
proven17 %>%
  ...
  gather(variable, valor, -geom) %>% 
  ggplot() + aes(fill = valor) + geom_sf(lwd = 0.2) +
  facet_wrap(~variable) + theme(text = element_text(size = 08)) +
  scale_fill_gradientn(colours = RColorBrewer::brewer.pal(9, name = 'Reds'), trans = 'log10') +
  geom_sf_text(aes(label=valor), check_overlap = T, size = 2)
```
...
#Rótulos: nombres de las provincias
proven17 %>%
  dplyr::select(contains('A qué cree usted que se debe la delincuencia en su barrio o comunidad: ¿Otra causa?'), TOPONIMIA) %>%
  gather(variable, valor, -geom, -TOPONIMIA) %>% 
  ggplot() + aes(fill = valor) + geom_sf(lwd = 0.2) +
  facet_wrap(~variable) + theme(text = element_text(size = 08)) +
  scale_fill_gradientn(colours = RColorBrewer::brewer.pal(9, name = 'Reds'), trans = 'log10') +
  geom_sf_text(aes(label=TOPONIMIA), check_overlap = T, size = 2)
```
...
``` r
proven17 %>% st_drop_geometry() %>%
  dplyr::select(TOPONIMIA, contains('A qué cree usted que se debe la delincuencia en su barrio o comunidad: ¿Otra causa?'))
##                 TOPONIMIA
## 1       DISTRITO NACIONAL
## 2                    AZUA
## 3                 BAORUCO
...
proven17 %>%
  ...
  facet_wrap(~variable) + theme(text = element_text(size = 08)) +
  scale_fill_gradientn(colours = RColorBrewer::brewer.pal(9, name = 'Reds')) +
  geom_sf_text(aes(label=pct), check_overlap = T, size = 2)
```
...
proven17 %>%
  ...
  group_by(TOPONIMIA) %>% 
  mutate(pct=round(valor/sum(valor)*100,2)) %>% dplyr::select(-valor) %>% 
  filter(variable %in% 'A qué cree usted que se debe la delincuencia en su barrio o comunidad: ¿Otra causa?: No') %>% 
  ggplot() + aes(fill = pct) + geom_sf(lwd = 0.2) +
  facet_wrap(~variable) + theme(text = element_text(size = 10)) +
  scale_fill_gradientn(colours = RColorBrewer::brewer.pal(9, name = 'Reds')) +
  geom_sf_text(aes(label=pct), check_overlap = T, size = 3)
...
proven17 %>% st_drop_geometry() %>%
  dplyr::select(contains('A qué cree usted que se debe la delincuencia en su barrio o comunidad: ¿Otra causa?')) %>%
  select_if(is.numeric) %>%  gather(variable, valor) %>%
  ggplot + aes(x=valor) %>% geom_histogram(bins=20) +
...
proven17 %>%
  ...
  ggplot() + aes(x = TOPONIMIA, y = valor, fill = variable, group = variable) +
  geom_col(position = 'dodge') + scale_y_continuous() +
  theme(axis.text.x = element_text(angle = 90), text = element_text(size = 08))  + coord_flip()
```
...
``` r
proven17 %>% st_drop_geometry() %>% filter(ENLACE=='0306') %>% 
  dplyr::select(contains('A qué cree usted que se debe la delincuencia en su barrio o comunidad: ¿Otra causa?'))
##   A qué cree usted que se debe la delincuencia en su barrio o comunidad: ¿Otra causa?: Si
## 1                                                                                      24
##   A qué cree usted que se debe la delincuencia en su barrio o comunidad: ¿Otra causa?: No
